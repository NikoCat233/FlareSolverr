...
def clean_files():
    try:
        shutil.rmtree(
            os.path.join(os.path.dirname(os.path.abspath(__file__)), os.pardir, "build")
        )
    except Exception:
        pass
    try:
        shutil.rmtree(
            os.path.join(os.path.dirname(os.path.abspath(__file__)), os.pardir, "dist")
        )
    except Exception:
        pass
    try:
        shutil.rmtree(
            os.path.join(
                os.path.dirname(os.path.abspath(__file__)), os.pardir, "dist_chrome"
            )
        )
    except Exception:
        pass
...
def compress_package():
    dist_folder = os.path.join(
        os.path.dirname(os.path.abspath(__file__)), os.pardir, "dist"
    )
    package_folder = os.path.join(dist_folder, "package")
    shutil.move(
        os.path.join(dist_folder, "flaresolverr"),
        os.path.join(package_folder, "flaresolverr"),
    )
    print("Package folder: " + package_folder)

    compr_format = "zip" if os.name == "nt" else "gztar"
    compr_file_name = (
        "flaresolverr_windows_x64" if os.name == "nt" else "flaresolverr_linux_x64"
    )
